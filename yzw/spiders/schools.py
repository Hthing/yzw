...
class SchoolsSpider(scrapy.Spider):
    ...
    start_urls = ['https://yz.chsi.com.cn/zsml/queryAction.do?ssdm={}&dwmc=&mldm={}&mlmc=&yjxkdm={}&xxfs=&zymc={}&pageno=1'
                          .format(st.ssdm, st.mldm, st.yjxkdm, st.zymc)]
        ...
        for url in self.start_urls:
            if st.ssdm == '':
                for province in st.provinceList:
                    url = re.sub(r'ssdm=&', 'ssdm=%s&' % province, url)
                    return self.judge_xkdm(url)
            else:
                return self.judge_xkdm(url)

    # 判断一级学科代码是否为空（为空不能直接查询）
    def judge_xkdm(self, url):
        if st.yjxkdm == '':
            for yjxkdm in self.firstClassSubjectIndex:
                url = re.sub(r'yjxkdm=&', 'yjxkdm=%s&' % yjxkdm, url)
                yield scrapy.Request(url, callback=self.parse_school_list)
        else:
            yield scrapy.Request(url, callback=self.parse_school_list)
    ...
    def parse_major(self, response):
        ...
        for test in testRange:
            ...
            majorDict['院校特性'] = self.__getSchoolFeature(majorDict['招生单位'][7:])
